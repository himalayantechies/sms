                    ...
                    <div class="eForm-layouts">
                        <form action="{{ route('teacher.profile.update') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="fpb-7">
                                ...
                                <input type="text" class="form-control eForm-control" id="eInputName" name="name"
                                    value="{{ auth()->user()->name }}" placeholder="Your Name" aria-label="Your Name" />
                            </div>
                            <div class="fpb-7">
                                <label for="eInputEmail" class="eForm-label">{{ get_phrase('Email') }}</label>
                                <input type="email" class="form-control eForm-control" id="eInputEmail" name="email"
                            ...
                            <div class="fpb-7">
                                ...
                                <input type="text" class="form-control eForm-control" id="designation" name="designation"
                                    value="{{ auth()->user()->designation }}" placeholder="Enter your designation" />
                            </div>
                            ...
                            <div class="fpb-7">
                                ...
                                @endphp
                                <input type="text" class="form-control eForm-control" id="eInputDate"
                                    name="eDefaultDateRange" value="{{ date('m/d/Y', $birthday) }}" />
                            </div>
                            <div class="fpb-7">
                                ...
                                <select name="gender" class="form-select eForm-select eChoice-multiple-without-remove"
                                    ...
                                    <option value="Male"
                                        @php strtolower(json_decode(auth()->user()->user_information, true)['gender']) == 'male' ? 'selected':''; @endphp>
                                        {{ get_phrase('Male') }}</option>
                                    <option value="Female"
                                        @php strtolower(json_decode(auth()->user()->user_information, true)['gender']) == 'female' ? 'selected':''; @endphp>
                                        {{ get_phrase('Female') }}</option>
                                </select>
                            </div>
                            <div class="fpb-7">
                                <label for="eInputPhone" class="eForm-label">{{ get_phrase('Phone Number') }}</label>
                                <input type="number" class="form-control eForm-control" id="eInputPhone" name="phone"
                                    value="{{ json_decode(auth()->user()->user_information, true)['phone'] }}"
                                    placeholder="00 (00) 12345 6789" aria-label="00 (00) 12345 6789" />
                            </div>
                            <div class="fpb-7">
                                <label for="eInputAddress" class="eForm-label">{{ get_phrase('Address') }}</label>
                                <input type="text" class="form-control eForm-control" id="eInputAddress"
                                    name="address"
