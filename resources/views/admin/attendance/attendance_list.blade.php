                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
                    ...
                    <div class="export-btn-area">
                        <a href="#" class="export_btn"
                            onclick="rightModal('{{ route('admin.take_attendance.open_modal') }}', '{{ get_phrase('Take Attendance') }}')">{{ get_phrase('Take Attendance') }}</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="eSection-wrap-2">
                <!-- Filter area -->
                <form method="GET" enctype="multipart/form-data" class="d-block ajaxForm"
                    action="{{ route('admin.daily_attendance.filter') }}">
                    <div class="att-filter d-flex flex-wrap">
                        <div class="att-filter-option">
                            <select name="month" id="month"
                                ...
                                <option value="Jan"{{ $page_data['month'] == 'Jan' ? 'selected' : '' }}>
                                    {{ get_phrase('January') }}</option>
                                <option value="Feb"{{ $page_data['month'] == 'Feb' ? 'selected' : '' }}>
                                    {{ get_phrase('February') }}</option>
                                <option value="Mar"{{ $page_data['month'] == 'Mar' ? 'selected' : '' }}>
                                    {{ get_phrase('March') }}</option>
                                <option value="Apr"{{ $page_data['month'] == 'Apr' ? 'selected' : '' }}>
                                    {{ get_phrase('April') }}</option>
                                <option value="May"{{ $page_data['month'] == 'May' ? 'selected' : '' }}>
                                    {{ get_phrase('May') }}</option>
                                <option value="Jun"{{ $page_data['month'] == 'Jun' ? 'selected' : '' }}>
                                    {{ get_phrase('June') }}</option>
                                <option value="Jul"{{ $page_data['month'] == 'Jul' ? 'selected' : '' }}>
                                    {{ get_phrase('July') }}</option>
                                <option value="Aug"{{ $page_data['month'] == 'Aug' ? 'selected' : '' }}>
                                    {{ get_phrase('August') }}</option>
                                <option value="Sep"{{ $page_data['month'] == 'Sep' ? 'selected' : '' }}>
                                    {{ get_phrase('September') }}</option>
                                <option value="Oct"{{ $page_data['month'] == 'Oct' ? 'selected' : '' }}>
                                    {{ get_phrase('October') }}</option>
                                <option value="Nov"{{ $page_data['month'] == 'Nov' ? 'selected' : '' }}>
                                    {{ get_phrase('November') }}</option>
                                <option value="Dec"{{ $page_data['month'] == 'Dec' ? 'selected' : '' }}>
                                    {{ get_phrase('December') }}</option>
                            </select>
                        </div>
                        <div class="att-filter-option">
                            <select name="year" id="year"
                                ...
                                <option value="{{ $year }}"{{ $page_data['year'] == $year ? 'selected' : '' }}>
                                    {{ $year }}</option>
                                <?php } ?>
                        ...
                        <div class="att-filter-option">
                            <select name="class_id" id="class_id"
                                ...
                                <option value="{{ $class['id'] }}"
                                    {{ $page_data['class_id'] == $class['id'] ? 'selected' : '' }}>{{ $class['name'] }}
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="att-filter-option">
                            <select name="section_id" id="section_id"
                                ...
                                <option value="{{ $section['id'] }}"
                                    {{ $page_data['section_id'] == $section['id'] ? 'selected' : '' }}>
                                    {{ $section['name'] }}</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                                ...
                                <ul class="dropdown-menu dropdown-menu-end eDropdown-menu-2">
                                    <li>
                                        <button class="dropdown-item" href="#"
                                            onclick="download_csv()">{{ get_phrase('CSV') }}</button>
                                    </li>
                                    <li>
                                        <button class="dropdown-item" href="#"
                                            onclick="Export()">{{ get_phrase('PDF') }}</button>
                                    </li>
                                </ul>
                            </div>
                        @endif
                ...
                @if (count($attendance_of_students) > 0)
                    <div
                        ...
                        <div class="att-report-summary order-1">
                            <h4 class="title">
                                {{ get_phrase('Attendance Report Of') . ' ' . date('F', $page_data['attendance_date']) . ', ' . date('Y', $page_data['attendance_date']) }}
                            </h4>
    ...
    <script type="text/javascript">
        ...
        var download_csv = function() {
            ...
            if (role_id != "" && month != "" && year != "") {
                ...
                var url = '{{ route('admin.dailyAttendanceFilter_csv', 'month,year,role_id') }}';
