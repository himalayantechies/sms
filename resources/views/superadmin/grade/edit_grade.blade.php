<div class="eoff-form">
    <form method="POST" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('superadmin.grade.update', ['id' => $grade->id]) }}">
        @csrf 
        <div class="form-row">
            <div class="fpb-7">
                <label for="grade_type" class="eForm-label">Grading type</label>
                <select id="grade_type" name="grade_type" class="form-select eForm-select eChoice-multiple-with-remove">
                    <option value="Letter Grading" @if ($grade->grade_type ="Letter Grading")selected @endif>Letter Grading </option>    
                    <option value="Percentage" @if ($grade->grade_type ="Percentage")selected @endif> Percentage</option>    
                </select>
            </div>
            <div class="fpb-7">
